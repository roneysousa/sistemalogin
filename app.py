...
@route('/') # @get('/login')
def login():
    return template('view/login.html')
...
def acao_login():
    ...
    return template('view/verificacao_login', sucesso=check_login(username, password), nome=username)

@error(404)
def error404(error):
    return template('view/pagina404')
